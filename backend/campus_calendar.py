...
class InMemoryCalendarManager:
    ...
    def create_event(self, user_id: str, event_data: Dict) -> Dict:
        ...
        # Parse ISO strings
        try:
            start = self._parse_iso_datetime(event_data['start_time'])
            end = self._parse_iso_datetime(event_data['end_time'])
        ...
        for ev in user_events:
            ev_start = self._parse_iso_datetime(ev['start_time'])
            ev_end = self._parse_iso_datetime(ev['end_time'])
    ...
    def get_user_events(self, user_id: str, start: Optional[datetime] = None, end: Optional[datetime] = None) -> List[Dict]:
        events = list(self.store.get(user_id, []))
    ...
    def find_free_slots(self, user_id: str, start: datetime, end: datetime, min_duration: int = 30) -> Dict:
        ...
        parsed = []
        for ev in events:
            parsed.append((self._parse_iso_datetime(ev['start_time']), self._parse_iso_datetime(ev['end_time'])))
        parsed.sort()
        slots = []
        cursor = start
        for s, e in parsed:
        ...
        class SimpleCalendar:
            ...
            def check_availability(self, start, end):
                for ev in self.events:
                    ev_start = InMemoryCalendarManager._parse_iso_datetime(ev['start_time'])
                    ev_end = InMemoryCalendarManager._parse_iso_datetime(ev['end_time'])
                    if not (end <= ev_start or start >= ev_end):
                        return False
                return True
